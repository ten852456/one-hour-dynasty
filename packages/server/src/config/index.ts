...
export function validateConfig(): void {
  ...
  if (config.nodeEnv === "production") {
    ...
    if (config.monad.payToAddress && !/^0x[a-fA-F0-9]{40}$/.test(config.monad.payToAddress)) {
  ...
  if (config.game.maxAgentsPerGame < config.game.minAgentsToStart) {
    errors.push("MAX_AGENTS_PER_GAME must be greater than or equal to MIN_AGENTS_TO_START");
  }
